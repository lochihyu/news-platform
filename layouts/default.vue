<template>
  <q-layout view="lHh lpR fff">
    ...
    <q-header elevated class="bg-white text-black" height-hint="64">
      <q-toolbar>
        ...
        <q-form @submit="onSearch">
          <q-input ref="searchInput" dense standout v-model="keywords" input-class="text-right" class="q-ml-md" placeholder="搜尋關鍵字">
            <template v-slot:append>
              <q-icon v-if="keywords === ''" name="search" @click="searchInput.focus()" />
              <q-icon v-else name="clear" class="cursor-pointer" @click="keywords = ''" />
            </template>
          </q-input>
        </q-form>
    ...
    <q-drawer show-if-above v-model="leftDrawerOpen" side="left" overlay behavior="mobile" elevated>
      <q-list>
        <q-item
            v-for="group in groups"
            :key="group.id"
...
async function loadGroups() {
  ...
  const { data, error } = await useAsyncData(() => $fetch(config.public.apiBaseUrl + '/apis/v1/groups'))
